 ...
 * Se cambio la validación para que los aprobadores (UPDATER) no vean los filtros de los casos. 
 */
if (current_user_get_access_level() > UPDATER/* REPORTER */)
	...
	<td class="form-title" colspan="<?php echo $col_count; ?>">
		...
		?> </span>
         <?php
                    ...
                    // Se cambió la condicion: current_user_get_access_level() > REPORTER por: current_user_get_access_level() > UPDATER
                    // para que el rol APROBADOR tampoco vea estos enlaces
...
?>
	<tr>
		<td class="left" colspan="<?php echo $col_count; ?>">
			<span class="floatleft">
<?php
   //jdholguin - se consulta el máximo permiso de un usuario logeado en Mantis

        $user_id = auth_get_current_user_id();
        $sql = "SELECT MAX(access_level) AS access_level FROM mantis_project_user_list_table WHERE user_id = " . $user_id;
        $resultado = db_fetch_array(db_query($sql));
        $current_user_max_access_level = $resultado['access_level'];

		if(($g_checkboxes_exist && ON == config_get( 'use_javascript' ))|| current_user_get_access_level() > REPORTER || $current_user_max_access_level > REPORTER) {
   ...
   //jdholguin - se agrega el or, para saber si el máximo rol del usuario es mayor que informador y le muestra los checkbox
		if( $g_checkboxes_exist || current_user_get_access_level() > REPORTER || $current_user_max_access_level > REPORTER ) {
?>
			<select name="action">
