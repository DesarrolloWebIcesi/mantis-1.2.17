...
//jdholguin - para ocultar los casos con estado "aprobado" de pendientes por aprobar
// y mostrar solo los de estado asignado
$t_hide_status_acknowledged = config_get( 'hide_status_acknowledged' );
$t_bug_assigned_status_threshold = config_get( 'bug_assigned_status_threshold' );
...
//jdholguin - agregado para darle un valor al índice "toacknowledge"
$c_filter['toacknowledge'] = array(
	FILTER_PROPERTY_CATEGORY => Array(
		'0' => META_FILTER_ANY,
	),
	FILTER_PROPERTY_SEVERITY_ID => Array(
	...
	FILTER_PROPERTY_STATUS_ID => Array(
		'0' => $t_bug_assigned_status_threshold,
	),
	FILTER_PROPERTY_HIGHLIGHT_CHANGED => $t_default_show_changed,
	FILTER_PROPERTY_REPORTER_ID => Array(
		'0' => META_FILTER_ANY,
...
$url_link_parameters['toacknowledge'] = FILTER_PROPERTY_STATUS_ID .'='. $t_bug_assigned_status_threshold .'&'. FILTER_PROPERTY_HANDLER_ID. '=' .$t_current_user_id. '&' . FILTER_PROPERTY_NOTE_USER_ID. '=' . META_FILTER_ANY . '&' . FILTER_PROPERTY_HIDE_STATUS_ID . '=' . $t_hide_status_acknowledged;
//fin jdholguin



/* rjaramillo 2011-12-19
 * Agregado para que en la caja "Asignados a mí (pendientes)" también muestre los casos con estado "Resuelto"
 */
if($t_box_title == "assigned")
{
    $c_filter[$t_box_title]['hide_status'] = array("90");
}
...
<tr bgcolor='#E6E6E6'>
  ...
  <td class="center" width="16%"><b>Fecha de entrega</b></td>
</tr>
...
<?php
  ...
  * rjaramillo - Richard Jaramillo Herrera
  * 2010-02-04
  * Agregado para consultar el campo Fecha de entrega y desplegarlo en la Vista
  * 2010-02-25 Agregado para desplegar el campo Fecha de envío y desplegarlo en la Vista
  */
  $sql = "SELECT value FROM mantis_custom_field_string_table WHERE field_id='32' AND bug_id='".$t_bug->id."';";
  $resultado = db_fetch_array( db_query( $sql ) );
  if($resultado['value'] != "DD-MON-AAAA HH:MM" && $resultado['value'] != "")
  {
    echo "
  <td><span class=\"small\">".$t_date_submitted."</span></td>
  <td><span class=\"small\"><center>".str_replace($mes_ingles, $mes_espanol, date("d-M-y", $resultado['value']))."</center></span></td>
    ";
  }
  else
  {
