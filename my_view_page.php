 ...
 * 2012-06-14
 * Se agrego la validación para que solo muestre las cajas para las aprobadores (UPDATER)
 */
if (current_user_get_access_level() > REPORTER && current_user_get_access_level() > UPDATER) {
    $t_boxes = config_get('my_view_boxes');
} elseif (current_user_get_access_level() == REPORTER) {
    $t_boxes = config_get('my_view_boxes_reporter');
} elseif (current_user_get_access_level() == UPDATER) {
    $t_boxes = config_get('my_view_boxes_updater');
}
