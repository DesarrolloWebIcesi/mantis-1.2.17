...
<?php
    ...
    // Se agregó el if para que el rol APROBADOR no vea a quién está asignado el caso
...
<?php }// END if(current_user_get_access_level() > UPDATER) ?>
<?php } ?>
...
<tr <?php echo helper_alternate_class() ?>>
 <td class="category">
  <?php if ( $t_require ) {
//jdholguin - se agrega el if para verificar que si es más que un actualizador, se muestre
//el * para saber que es un campo obligatorio
 if (current_user_get_access_level() > UPDATER) {
 ?>
  ...
  <?php } 
	...
	<td>
		<?php
			//jdholguin - se agrega el if para verificar si el usuario es más que un
           //actualizador para dejarle modificar el campo, en caso contrario, solo se muestra el valor
           if (current_user_get_access_level() > UPDATER) {
