...
function html_head_javascript() {
    ...
    // rjaramillo 2012-08-17
   // Agregado para cargar el archivo JavaScript para validar el correo electrónico en el Reporte de casos. Caso SGS: 62281
        html_javascript_link('validacion.js');
    }
}
...
function html_login_info() {
   ...
   /* rjaramillo 2011-12-01
      Modificado para que los Informadores no puedan cambiar el Proyecto */
    // if( $t_show_project_selector ) {
    if ($t_show_project_selector && current_user_get_access_level() > REPORTER) {
		echo '<form method="post" name="form_set_project" action="' . helper_mantis_url( 'set_project.php' ) . '">';
		# CSRF protection not required here - form does not result in modifications
       
        //jdholguin - se agrega un if para saber si el usuario que está es más que actualizador para mostar el listado
        //de proyectos
        if (current_user_get_access_level() > UPDATER) {
...
function print_menu() {
	if( auth_is_user_authenticated() ) {
         ...
         * 2010-02-02
         * Agregado para que los informadores no tenga esta opción de menú
         * jlozano - Juan Manuel Lozano Sardi
         * Se agrego el rol aprobador (UPDATER) para que no pueda ver la opción de ver casos 
         */
        if (current_user_get_access_level() > UPDATER/* REPORTER && current_user_get_access_level() > UPDATER */)
	   ...
	   /**
         ...
         * Agregado para que los informadores no tenga esta opción de menú
         * jlozano - Juan Manuel Lozano Sardi
         * Se agrego el rol aprobador (UPDATER) para que no pueda ver la opción de mi cuenta   
         */
        if (current_user_get_access_level() > UPDATER/* REPORTER && current_user_get_access_level() > UPDATER */)  	
         if( OFF == $t_protected ) {
...
function html_button_bug_update( $p_bug_id ) {
  //jlozano 19-06-2012 se agregó el and para que si el usuario pertenece al rol aprobador (UPDATER) no muestre el botórn de editar.
	if( access_has_bug_level( config_get( 'update_bug_threshold' ), $p_bug_id ) && current_user_get_access_level() > UPDATER ) {
...
function html_button_bug_assign_to( $p_bug ) {
  //jlozano 19-06-2012 se agregó el if para que si el usuario conectado pertence al rol aprobador (UPDATER) no muestre ninguna de las opciones de Asignar A.
   if (current_user_get_access_level() > UPDATER) {
...
function html_button_bug_monitor( $p_bug_id ) {
  //jlozano 19-06-2012 se agregó el and para validar que si el usuario pertenece al rol aprobador (UPDATER) entonces no muestre el boton de monitorizar.	
  if( access_has_bug_level( config_get( 'monitor_bug_threshold' ), $p_bug_id )&& current_user_get_access_level() > UPDATER ) {
...
function html_buttons_view_bug_page( $p_bug_id ) {
	...
	if ( !$t_readonly || config_get('allow_reporter_close')){
       // rjaramillo 2012-06-29
       // se agregó la variable $reporter_id y el if para que el rol APROBADOR no tenga la opción de aprobación para los casos reportados por él
        $reporter_id = bug_get_field($p_bug_id, 'reporter_id');
        if (current_user_get_access_level() != UPDATER ||
                (current_user_get_access_level() == UPDATER && $reporter_id != auth_get_current_user_id()) ||
                (current_user_get_access_level() == UPDATER && $reporter_id == auth_get_current_user_id() && $t_status == 80)) { 
		echo '<td class="center">';
