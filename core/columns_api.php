...
function print_column_selection($p_bug, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE) {
    ...
    //jlozano 19-06-2012 Se agrego la condición or para el caso en el que si el usuario pertenece al rol aprobador (UPDATER) entonces muestre el checkbox de mostrar casos
    //jdholguin - se agrega el or, para saber si el máximo rol del usuario es mayor que informador y le muestra los checkbox
    echo '<td>';
    if ((access_has_bug_level($t_update_bug_threshold, $p_bug->id) || current_user_get_access_level() > REPORTER) || $current_user_max_access_level > REPORTER) {
...
function print_column_edit( $p_bug, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE ) {
    ...
    echo '<td>';
//jlozano 19-06-2012 Se agregó el and para verificar que si el usuario conecatdo pertenece al rol aprobador (UPDATER) entonces no muestre el icono de editar de mostrando casos.
    if (!bug_is_readonly($p_bug->id) && access_has_bug_level(config_get('update_bug_threshold'), $p_bug->id) && current_user_get_access_level() > UPDATER) {
